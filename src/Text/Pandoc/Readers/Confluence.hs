...
confluence :: GenParser Char () (Attributed Block, Maybe Preamble, [Attributed Block])
confluence = do
...
readConfluence :: ParserState -> String -> P.Pandoc
readConfluence _ text = case parse confluence "<input>" text' of
    Left e -> error $ show e
    Right x -> pandoc x
  where
    text' = text ++ "\n\n"
